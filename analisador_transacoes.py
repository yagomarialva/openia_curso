...
def gerar_parecer(transacao):
    print("2. Gerando parecer para transação ", transacao["id"])
    prompt_sistema = f"""
...
for uma_transacao in transacoes_analisadas["transacoes"]: 
    if uma_transacao["status"] == "Possível Fraude": 
        ...
        # Armazenando informações relevantes
        id_transacao = uma_transacao["id"]
        produto_transacao = uma_transacao["nome_produto"]
        status_transacao = uma_transacao["status"]

        # Criando o conteúdo para salvar em um arquivo
        conteudo_arquivo = f"""
        Recomendação: {recomendacao}

        Detalhes: A transação ID {id_transacao}, classificada como 'Crédito' no valor de {uma_transacao["valor"]}, foi sinalizada como '{status_transacao}'. O produto associado à transação é um {produto_transacao}.
        """

        # Salvando em um arquivo de texto
        nome_arquivo = f"transacao-{id_transacao}-{produto_transacao}-{status_transacao}.txt"
        salva(nome_arquivo, conteudo_arquivo)